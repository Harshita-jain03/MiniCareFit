...
export async function GET(req: NextRequest) {
  console.log('➡️ GET /admin/student/todo');

  try {
    const token = req.headers.get("authorization");

    const { data, status } = await fetchWithTokenRetry(API_URLS.TODO.GET_ALL, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': token || '',
...
export async function POST(request: NextRequest) {
  console.log('➡️ POST /admin/student/task');
  ...
  try {
    ...
    const finalUrl = API_URLS.TODO.CREATE; // assumes your backend expects this
